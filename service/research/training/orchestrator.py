...
from service.research.training.genomes import (
    mutate_genome_for_model,
    normalize_genome_for_model,
    random_genome_dict_for_model,
)
from service.research.training.population import (
    build_generation_statistics,
    build_next_population,
)
from service.research.training.updates import TrainingUpdateHub
...
class TrainingRuntime:
    """Application-owned dependencies and mutable state for one training service."""
    ...
    def bot_service_client(self) -> BotServiceClient:
        if self.bot_client_factory is not None:
            return self.bot_client_factory()
...
def _record_heartbeat(runtime: TrainingRuntime, session_id: str, phase: str):
...
async def reconcile_stalled_training_sessions(runtime: TrainingRuntime,
...
async def training_watchdog_loop(runtime: TrainingRuntime,
                                 interval_seconds: int = 30,
                                 stale_after_seconds: int = 600):
    while True:
        await reconcile_stalled_training_sessions(runtime, stale_after_seconds)
        await asyncio.sleep(interval_seconds)


async def start_training_session(runtime: TrainingRuntime, ruleset: str,
                                 bot_count: int, generations: int,
                                 base_genome_id: str,
    ...
    db.create_training_batch(session_id, {
        ...
        "config": {
            ...
            "elite_count": session["elite_count"],
            "selection_size": session["selection_size"],
            "games_per_generation": session["games_per_generation"],
        },
    })
    active_training_sessions[session_id] = session
    _record_heartbeat(runtime, session_id, "starting")
    ...
    await _trigger_next_game(runtime, session_id)
    return session_id


async def cancel_training_session(runtime: TrainingRuntime, session_id: str,
...
async def _trigger_next_game(runtime: TrainingRuntime, session_id: str):
    await _start_generation_games(runtime, session_id)


async def _start_generation_games(runtime: TrainingRuntime, session_id: str):
    active_training_sessions = runtime.sessions
    session = active_training_sessions.get(session_id)
    if not session:
        return
    ...
    for attempt in range(1, session["games_per_generation"] + 1):
        tasks.append(asyncio.create_task(
            _start_generation_game_attempt(runtime, session_id, attempt)))
    await asyncio.gather(*tasks)


async def _start_generation_game_attempt(runtime: TrainingRuntime, session_id: str,
                                         attempt: int):
    active_training_sessions = runtime.sessions
    db = runtime.database
    session = active_training_sessions.get(session_id)
    ...
    if result.ok:
        ...
        _record_heartbeat(runtime, session_id, "running")
    ...
    await _record_terminal_game_attempt(
        runtime, session_id, game_id, [],
        f"Bot service spawn failed: {result.error_message}")


async def handle_training_game_ended(runtime: TrainingRuntime, game_id: str,
...
async def _record_terminal_game_attempt(runtime: TrainingRuntime,
                                        training_session_id: str, game_id: str,
                                        entries: list,
    ...
    async with session.setdefault("generation_lock", asyncio.Lock()):
        ...
        start_next_generation = await _complete_generation_locked(
            runtime, training_session_id, session)

    if start_next_generation:
        await _trigger_next_game(runtime, training_session_id)


async def _complete_generation_locked(runtime: TrainingRuntime,
                                      training_session_id: str,
                                      session: dict) -> bool:
    active_training_sessions = runtime.sessions
    db = runtime.database
    entries = session["fitness_entries"]
    _record_heartbeat(runtime, training_session_id, "aggregating_generation")
