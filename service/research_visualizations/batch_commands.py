...
class TradingAndContestingPerGameCommand(VisualizationCommand):
    ...
    title = "Trading and Contesting Per Game"
    description = "Two-series chart of trade and contest counts by training game."
    ...
    def render(self, context):
        ...
        figure, axes = plt.subplots(figsize=(8, 4))
        axes.bar([x - 0.2 for x in x_positions], trades, width=0.4, label="Trades")
        axes.bar([x + 0.2 for x in x_positions], contests, width=0.4, label="Contests")
